...
if [ ! -d "/opt/teiid" ]; then
	...
	sh /opt/teiid/bin/vault.sh -k /opt/teiid/vault/vault.keystore -p vault22 -e /opt/teiid/vault -i 120 -s 1234abcd -v vault -b keystore -a password -x Password1!
	...
	chown -R teiid:teiid /var/log/teiid
	chmod -R 755 /var/log/teiid



	echo "Register teiid service"
	ln -s /opt/teiid/docs/contrib/scripts/init.d/wildfly-init-redhat.sh /etc/init.d/teiid.sh
	cp /vagrant_data/teiid/wildfly.conf /etc/default/wildfly.conf
